...
console.log("IMAGE_BASE_URL:", IMAGE_BASE_URL);
  ...
  const toastMsg = (msg, status) => {
    toast(msg, {
      type: status === 1 ? 'success' : 'error', // ✅ Fixed: was (status === true) which never worked since status is 1 or 0
      theme: 'dark',
    });
  };
  ...
  const fetchColor = async (color_id = null) => {
    let colorLink = API_URL + Color_URL;
    if (color_id) {
      colorLink += `/${color_id}`;
    }
    try {
      const success = await axios.get(colorLink);
      if (color_id) {
        // Single color — return it, don't overwrite allColor list
        return success.data.color;
      } else {
 ...
 const fetchCategory = async (category_id = null) => {
  ...
  try {
    const success = await axios.get(categoryLink);
