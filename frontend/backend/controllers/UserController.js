...
class UserController {
  ...
  moveWishlistToDb(data, userId) {
   ...
   // ✅ NEW: Add a shipping address to a user's profile
  addAddress(userId, addressData) {
    return new Promise(async (resolve, reject) => {
      try {
        const user = await UserModel.findByIdAndUpdate(
          userId,
          { $push: { shipping_address: addressData } },
  ...
  deleteAddress(userId, addressIndex) {
    return new Promise(async (resolve, reject) => {
      try {
        const user = await UserModel.findById(userId);
        if (!user) return reject({ msg: "User not found", status: 0 });
 
        user.shipping_address.splice(addressIndex, 1);
        await user.save();
