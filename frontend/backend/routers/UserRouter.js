...
// ✅ NEW: Add a new shipping address for a user
UserRouter.post("/add-address/:userId", (req, res) => {
    const result = new UserController().addAddress(req.params.userId, req.body);
    result.then(success => res.send(success)).catch(error => res.send(error));
});
 
// ✅ NEW: Delete a shipping address by index
UserRouter.delete("/delete-address/:userId/:index", (req, res) => {
    const result = new UserController().deleteAddress(req.params.userId, Number(req.params.index));
    result.then(success => res.send(success)).catch(error => res.send(error));
});
